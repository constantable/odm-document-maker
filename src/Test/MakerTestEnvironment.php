...
echo json_encode($missingDependencies);
    ...
    {
        MakerTestProcess::create('composer require --dev constantable/odm-document-maker', $projectDirectory)
            ->run()
        ;
    ...
    private function addMakerBundleRepoToComposer(string $composerJsonPath): void
    {
        ...
        $composerJson['repositories']['constantable/odm-document-maker'] = [
            'type' => 'path',
            'url' => sprintf('%s%smaker-repo', $this->cachePath, \DIRECTORY_SEPARATOR),
            'options' => [
                'versions' => [
                    'constantable/odm-document-maker' => '9999.99', // Arbitrary version to avoid stability conflicts
                ],
            ],
        ];
