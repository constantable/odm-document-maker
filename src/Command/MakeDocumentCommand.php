...
namespace Constantable\OdmDocumentMaker\Command;
...
use Constantable\OdmDocumentMaker\Doctrine\DocumentClassGenerator;
use Constantable\OdmDocumentMaker\Doctrine\DocumentEmbedding;
use Constantable\OdmDocumentMaker\Doctrine\DocumentRegenerator;
use Constantable\OdmDocumentMaker\Doctrine\DocumentRelation;
use Constantable\OdmDocumentMaker\Doctrine\ODMDependencyBuilder;
...
final class MakeDocumentCommand extends AbstractMaker implements InputAwareMakerInterface
{
    ...
    {
        return 'constantable:make-document';
    }
    ...
    private function askRelationDetails(ConsoleStyle $io, string $generatedDocumentClass, string $type, string $newFieldName): DocumentRelation
    {
        ...
        $askFieldName = fn (string $targetClass, string $defaultValue) => $io->ask(
            ...
            function ($name) use ($targetClass) {
                ...
                // same constantable:make-document run. property_exists() only knows about
            ...
            if (class_exists($targetDocumentClass)) {
                ...
                if (!$manipulator->isEmbeddedDocument()) {
                    $io->error([sprintf(
                        ...
                        Str::getShortClassName($targetDocumentClass)), 'Please pick a correct EmbeddedDocument class or run constantable:make-document --embedded to create a new one.']);
                    $targetDocumentClass = null;
                    continue;
                }
            }
    ...
    private function askRelationType(ConsoleStyle $io, string $documentClass, string $targetDocumentClass): mixed
    {
